            ...
            do iX = -i_for_Re,i_for_Re  
                do iY = -i_for_Re,i_for_Re  
                    ...
                    if(iX == 0 .and. iY == 0) then
                        ...
                        w_gradient = weight_function(distance,Re_for_gradient)
                    end if
...
module calculation_module
    ...
    subroutine set_source_term()
        ...
        do i = i_start,i_finish
            ...
            !表面粒子はソース項0、それ以外の場合でソース項を計算
            if (boundary_condition(i) == inner_particle) then
                source_term(i) = relaxation_coefficient_for_pressure*(1.0_8/(time_interval*time_interval))*((number_density(i)-n0_for_number_density)/n0_for_number_density)
            end if
        end do
        !$omp end parallel do

        call allgather_real_vector(source_term)

    end subroutine set_source_term
    
    subroutine apply_matrix_to_vector(vector_in,matrix_applied_vector)
        !ポアソン方程式に現れる行列をベクトルに作用させるルーチン
        implicit none
        real(8),intent(in)::vector_in(:)
        real(8),intent(out)::matrix_applied_vector(:)

        integer :: i,j
        real(8) :: alpha
        real(8) :: beta
        real(8) :: cij,xij,yij,zij
        real(8) :: distance2,distance

        matrix_applied_vector(:) = 0.0
        alpha = 2.0*real(dimention,8)/(n0_for_laplacian*lambda_0*fluid_density)
        beta = compressibility/(time_interval*time_interval)

        !$omp parallel do private(j,xij,yij,zij,distance,distance2,cij)
        do i = i_start, i_finish

            !自由表面粒子については後のcg法で、ずっと圧力が0となるようにする。
            if (boundary_condition(i) /= inner_particle) then
                cycle
            end if
            matrix_applied_vector(i) = beta*vector_in(i)
            ...
            do j=1,number_of_particles
                ...
                if (boundary_condition(j) == dummy) then
                    cycle
                end if
                ...
                if (distance2 >= Re_for_laplacian*Re_for_laplacian  .or. distance2 <= 0.0) then
                    cycle
                end if

                distance = sqrt(distance2)
                cij = alpha * weight_function(distance,Re_for_laplacian)
                matrix_applied_vector(i) = matrix_applied_vector(i) + cij*(vector_in(i)-vector_in(j))

            end do
        end do
        !$omp end parallel do

        call allgather_real_vector(matrix_applied_vector)

    end subroutine apply_matrix_to_vector

    real(8) function inner_product_of_vector(vector1,vector2)
        implicit none
        real(8),intent(in)::vector1(:),vector2(:)
        integer :: i

        inner_product_of_vector = 0.0

        !$omp parallel do reduction(+:inner_product_of_vector)
        do i = 1,number_of_particles
            if (boundary_condition(i)==inner_particle)then
                inner_product_of_vector = inner_product_of_vector + vector1(i)*vector2(i)
            end if
        end do
        !$omp end parallel do
        
    end function inner_product_of_vector

    subroutine calculate_pressure_by_cg()
        !ポアソン方程式を共役勾配法で解く！
        implicit none

        real(8),allocatable :: residual(:)
        real(8),allocatable :: direction(:)
        real(8),allocatable :: matrix_applied_direction(:)
        real(8) :: alpha, beta,denominator,numerator
        integer :: i,iterr
        real(8) :: tolerance2
        real(8) :: residual_norm_2

        allocate(residual(number_of_particles))
        allocate(direction(number_of_particles))
        allocate(matrix_applied_direction(number_of_particles))

        tolerance2 = cg_relative_tolerance*cg_relative_tolerance*inner_product_of_vector(source_term,source_term)

        particle_pressure(:) = 0.0

        call apply_matrix_to_vector(particle_pressure,matrix_applied_direction)
        !$omp parallel do
        do i = 1,number_of_particles
            if(boundary_condition(i)==inner_particle) then
                residual(i)=source_term(i)-matrix_applied_direction(i)
        ...
        do iterr = 1,cg_max_iterr
            call apply_matrix_to_vector(direction,matrix_applied_direction)
            denominator = inner_product_of_vector(direction,matrix_applied_direction)
            if (abs(denominator) <= tiny(denominator)) then
                exit
            end if
            numerator = inner_product_of_vector(residual,residual)
            alpha = numerator/denominator

            !$omp parallel do
            do i = 1,number_of_particles
                if (boundary_condition(i)==inner_particle) then
                    particle_pressure(i) = particle_pressure(i) + alpha*direction(i)
                    residual(i) = residual(i)-alpha*matrix_applied_direction(i)
                else
                    particle_pressure(i) = 0.0
                    residual(i) = 0.0
                end if
            ...
            !Break条件
            residual_norm_2 = inner_product_of_vector(residual,residual)
            if (residual_norm_2 <= tolerance2) then
                exit
            end if

            beta = residual_norm_2/numerator
            !$omp parallel do
            do i = 1,number_of_particles
                if(boundary_condition(i)==inner_particle) then
        ...
        deallocate(matrix_applied_direction)

    end subroutine calculate_pressure_by_cg
    ...
    subroutine set_minimum_pressure()
        implicit none
        integer :: i,j
        real(8) :: xij,yij,zij
        real(8) :: distance2

        minimum_pressure(:) = 0.0_8

        !$omp parallel do private(j,xij,yij,zij,distance2)
        do i = i_start,i_finish
            if (particle_type(i) == dummywall) then
                cycle
            end if

            minimum_pressure(i) = particle_pressure(i)
            ...
            do j = 1, number_of_particles
                ...
                if (distance2 >= Re_for_gradient*Re_for_gradient) cycle

                if (minimum_pressure(i) > particle_pressure(j)) then
                    minimum_pressure(i) = particle_pressure(j)
                end if
            end do
        end do
        !$omp end parallel do

        call allgather_real_vector(minimum_pressure)

    end subroutine set_minimum_pressure
    ...
    subroutine calpressuregradient()
        ...
        do i = i_start,i_finish
            ...
            do j = 1, number_of_particles
                ...
                distance = sqrt(distance2)
                w = weight_function(distance, Re_for_gradient)
                pij = (particle_pressure(j) - minimum_pressure(i)) / distance2
...
module mainloop
    ...
    subroutine mainloopofsimulation()
        ...
        mainloop : do  timestep= 0,max_timestep
            ...
            !圧力の計算
            call calculate_pressure_by_cg()
            !圧力勾配計算の前処理
            call remove_negative_pressure()
            call set_minimum_pressure
            !圧力項
